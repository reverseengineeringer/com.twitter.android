  ...
  implements FragmentManager.BackStackEntry, Runnable
{
  ...
  {
    int n = 0;
    ensureFragmentsAreInitialized(paramSparseArray2);
  ...
  private void ensureFragmentsAreInitialized(SparseArray<Fragment> paramSparseArray)
  {
    int j = paramSparseArray.size();
    int i = 0;
    while (i < j)
    {
      Fragment localFragment = (Fragment)paramSparseArray.valueAt(i);
      if (mState < 1)
      {
        mManager.makeActive(localFragment);
        mManager.moveToState(localFragment, 1, 0, 0, false);
      }
      i += 1;
    }
  }
